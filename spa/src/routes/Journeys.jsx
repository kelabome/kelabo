    ...
    <Link className={'row anim-in' + (delay ? ` anim-in-d${Math.min(delay, 4)}` : '')} to={`/journeys/${j.journeyId}`}>
      ...
      <div className="row-main">
        <div className="row-title">
          {j.title}
          {/* Unread in a leg of this journey (docs 20 §19.3). A mention
              outranks a plain count — "somebody wants you" and "there is
              something here" are different messages. */}
          {j.mentions > 0 ? (
            <span className="leg-badge leg-badge-mention" title={`${j.mentions} unread mention${j.mentions === 1 ? '' : 's'}`}>
              @{j.mentions}
            </span>
          ) : j.unread > 0 ? (
            <span className="leg-badge" title={`${j.unread} unread message${j.unread === 1 ? '' : 's'}`}>{j.unread}</span>
          ) : null}
...
export default function Journeys() {
  // The list and the rail's badge come from one fetch, owned by AppShell —
  // which polls it (docs 20 §19.3). Fetching here as well would mean two
  // timers asking the same question and disagreeing for a few seconds at a
  // time, on the one page where the answer is the whole point.
  const { journeys: data, journeysError: error } = useAppData()
